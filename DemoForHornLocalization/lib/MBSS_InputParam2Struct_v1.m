function s = MBSS_InputParam2Struct_v1(angularSpectrumMeth,c,fftSize_sec,blockDurationUser_sec,blockOverlapUser_percent,pooling,azBound,elBound,gridRes,alphaRes,minAngle,nsrc,fs,normalizeSpecInst,specDisplay,enableWienerFiltering,wienerMode,freqRange,micPos,isArrayMoving,subArray,sceneTimeStamps,angularSpectrumDebug,carhorn)
...
%% 新的结构体carhorn
xBound=carhorn.xBound;
yBound=carhorn.yBound;
iz=carhorn.iz;
xRes=carhorn.xRes;
yRes=carhorn.yRes;
xRes2=carhorn.xRes2;
...
minX=carhorn.minX;
minY=carhorn.minY;
FlagforVAD=carhorn.FlagforVAD;
FreIdx_last_All=carhorn.FreIdx_last_All;
FreIdx_last_All2=carhorn.FreIdx_last_All2;
show = carhorn.show;
model = carhorn.model;
...
if(isempty(angularSpectrumMeth))
    % default init : GCC_PHAT_BaseXY
    s.angularSpectrumMeth = 'GCC-PHAT-BaseXY';
elseif(~any(strcmp(angularSpectrumMeth, {'GCC-PHAT-BaseXY' 'MUSIC-BaseXY' 'GCC-PHAT' 'GCC-NONLIN' 'MVDR' 'MVDRW' 'DS' 'DSW' 'DNM'})))
...
if(any(strcmp(s.angularSpectrumMeth, {'GCC-PHAT-BaseXY' 'MUSIC-BaseXY' 'GCC-PHAT','GCC-NONLIN'})))
...
s.FlagforVAD=FlagforVAD;
if(any(strcmp(s.angularSpectrumMeth, {'MUSIC-BaseXY'})))
    s.FreIdx_last_All=FreIdx_last_All;
    s.FreIdx_last_All2=FreIdx_last_All2;
else
    s.FreIdx_last_All=FreIdx_last_All2;
    s.FreIdx_last_All2=FreIdx_last_All2;
end
if isempty(FreIdx_last_All)==0
    s.freqBins=FreIdx_last_All;
    s.freqBins2=FreIdx_last_All2;
end
...
s.xscan = (xBound(1) : xRes : xBound(2))';
s.yscan = (yBound(1) : yRes : yBound(2))';
s.xRes=xRes;
s.yRes=yRes;
s.iz=iz;
...
s.xGrid = repmat(s.xscan,ny,1)';
s.yGrid   = (reshape((repmat(s.yscan,1,nx))',1,nx*ny));
